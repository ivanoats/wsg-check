...
| `list_guidelines` | Optional `category`, `testability`, `query` (text in the title, slug, or description) | The guidelines in the targeted WSG release, each with its slug, number, category, testability, and how many `check_url` checks implement it. No network access. |
